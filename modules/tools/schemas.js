...
const COBRA_TOOLS = [
  ...
  // ── L'esperienza: quello che si e' imparato usando un sito ──
  //
  // Un agente generalista arriva su booking.com per la millesima volta e non
  // sa niente piu' della prima. COBRA lavora sempre sugli stessi venti
  // portali: su quelli puo' sapere cose che nessun modello generico sa.
  { type:'function', function:{ name:'cosa_so_del_sito', description:'Cosa si e\' gia\' imparato su questo sito: quali strategie hanno funzionato, quali no, che ostacoli mette, quanto e\' lento. CHIAMALO prima di lavorare su un portale che potresti aver gia\' visto.', parameters:{ type:'object', properties:{ url:{ type:'string', description:'l\'indirizzo o il dominio; se manca si usa la pagina aperta' } } } } },
  { type:'function', function:{ name:'annota_sul_sito', description:'Segna una cosa imparata su un sito, perche\' valga anche le prossime volte. Se una strada NON ha funzionato e un\'altra si\', usa invece_di: quella coppia e\' l\'informazione piu\' preziosa, evita di ripetere lo stesso errore per sempre.', parameters:{ type:'object', properties:{ url:{ type:'string' }, cosa:{ type:'string', description:'cosa hai imparato, in una frase' }, invece_di:{ type:'string', description:'la strada che aveva fallito, se ce n\'era una' }, tipo:{ type:'string', enum:['funziona','non_funziona','ostacolo','osservazione'] }, perche:{ type:'string' } }, required:['cosa'] } } },
  { type:'function', function:{ name:'impara_procedura', description:'Registra una procedura che si ripetera\': "preventivo UPS", "controllo ritardi DHL". Descrivi i campi per SIGNIFICATO ("origine", "peso"), mai con selettori CSS, e usa {{nome}} per i valori che cambiano ogni volta. Cosi\' regge anche quando il sito cambia aspetto.', parameters:{ type:'object', properties:{ nome:{ type:'string' }, quando:{ type:'string', description:'quando si usa, in una frase' }, sito:{ type:'string' }, passi:{ type:'string', description:'JSON: [{"cosa":"...","dove":"significato del campo","valore":"{{parametro}}"}]' } }, required:['nome','passi'] } } },
  { type:'function', function:{ name:'usa_procedura', description:'Prepara una procedura gia\' imparata coi valori di adesso. Se manca un valore lo dice invece di inventarlo.', parameters:{ type:'object', properties:{ nome:{ type:'string' }, valori:{ type:'string', description:'JSON: {"partenza":"Milano","peso":"25"}' } }, required:['nome'] } } },
  { type:'function', function:{ name:'elenco_procedure', description:'Le procedure gia\' imparate.', parameters:{ type:'object', properties:{} } } },
